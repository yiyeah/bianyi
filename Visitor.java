...
class Visitor extends lab1BaseVisitor<Void>{
    ...
    public Void visitCompUnit(lab1Parser.CompUnitContext ctx) {
        init();
       curBlock.saveBuf("declare i32 @getint()",true);
       curBlock.saveBuf("declare void @putint(i32)",true);
       curBlock.saveBuf("declare i32 @getch()",true);
       curBlock.saveBuf("declare void @putch(i32)",true);
       visit(ctx.funcDef());
        return null;
    }
    ...
    public Void visitFuncDef(lab1Parser.FuncDefContext ctx) {
        curBlock.saveBuf("define dso_local ",false);
        visit(ctx.funcType());
        curBlock.saveBuf("@"+ctx.Ident()+" () ",false);
        visit(ctx.block());
        return null;
    }
    ...
    public Void visitBlock(lab1Parser.BlockContext ctx) {
        if(ctx.getParent().children.size()== 5){
            curBlock.saveBuf("{ \n",false);
    ...
    public Void visitStmt(lab1Parser.StmtContext ctx) { 
        switch (ctx.children.size()) {
            ...
            case 7:{
                ...
                tmp = ctx.stmt().get(1);         
                visit(tmp);
                if(curBlock!=root && curBlock.parBlock.destBlock!=null&&curBlock.parBlock.destBlock.equals(curBlock)){
                ...
                String dest = ++counter+":";
                if(curBlock.jmpTrueBlock.jmpTrueBlock==null)
                    curBlock.jmpTrueBlock.saveBuf("br label %"+counter,true);
                if(curBlock.jumFalseBlock.jmpTrueBlock==null)
    ...
    public static void exeFunc(String funcName, String param){
        ...
        if(func.retType.equals("void")){
            ...
            curBlock.saveBuf("call void @"+func.funcName+"("+"i32 "+param+")", true);
        }

        else if(func.retType.equals("int")){
            curBlock.saveBuf("%"+ ++counter+" = call i32 @"+func.funcName+"("+param+")", true);
            nodenumber = "%"+counter;
        }
    }
